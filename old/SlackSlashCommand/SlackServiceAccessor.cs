...
using SlackSlashCommand.Entities.Block;
using System;
   ...
   public class SlackServiceAccessor
    {
            ...
            string triggerId)
        {
            ...
            var contents = await response.Content.ReadAsStringAsync();
            //var messageDetails = JsonConvert.DeserializeObject<DialogResponse.RootObject>(contents);

            return contents;
        }

        public async Task<string> SendCodeSnippetAsync(
            string token,
            string channel,
            string title,
            string snippet,
            string user)
        {
            var urlEncode = System.Web.HttpUtility.UrlEncode(snippet);

            var client = GetHttpClientWithAuthHeader(
                token: token,
                mediaType: "application/json");

            string url = "https://slack.com/api/files.upload?token=" + token + $"&channels={channel}&title=SlashCommands.txt&content={urlEncode}&filetype=text&pretty=1";
        ...
        public async Task<string> SendPostEphemeralAsync(
            ...
            string message)
        {
            var urlEncode = System.Web.HttpUtility.UrlEncode(message);
